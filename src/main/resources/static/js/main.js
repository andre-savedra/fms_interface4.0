...
function setSpinnerButton (element) {
  ...
  element.innerHTML =
    actual +
    '<span id="spinBtn" class="spinner-border spinner-border-sm" role="status" aria-hidden="true" style="margin-left: 4px; margin-bottom: 4px;"></span>'
}
...
function submitCustom (element) {
  ...
  let myordername = document.querySelector('#' + codeSelector + '_program')
    .value
...
function submitPart (element) {
  ...
  process_validated['steps'].forEach(element => {
    listStep.push(new StepOrder(element.name, element.gcode, element.machine))
  })
...
async function loadDataXDK () {
  fetch(defaultUrl + 'xdk', {
    ...
    .then(response => {
      ...
      console.log('XDK RESPONSE')
      console.log(response.response)
      const data = response.response.reverse();
      data.map((xdk, index) => {
        const accRow = [index + 1, xdk.accx, xdk.accy, xdk.accz]
        xdkDataAcc.push(accRow)

        const girRow = [index + 1, xdk.gyx, xdk.gyy, xdk.gyz]
        xdkDataGir.push(girRow)
        
        const lightRow = [index + 1, xdk.light]
        xdkDataLum.push(lightRow)
        
        const tpuRow = [index + 1, xdk.temperature, xdk.pressure, xdk.humidity]
        xdkDataTPU.push(tpuRow)
...
function refreshChartsDashboards () {
  ...
  drawTimeline(
    document.getElementById('timeline_turn_chartXdk'),
    ['Acel-X', 'Acel-Y', 'Acel-Z'],
    xdkDataAcc,
    optionsConfig('XDK: Acelerômetro Eixo X,Y,Z', 'X,Y,Z')
  )
  drawTimeline(
    document.getElementById('timeline_turn_chartXdk2'),
    ['Gir-X', 'Gir-Y', 'Gir-Z'],
    xdkDataGir,
    optionsConfig('XDK: Giroscópio Eixo X,Y,Z', 'X,Y,Z')
  )
  drawTimeline(
    document.getElementById('timeline_turn_chartXdk3'),
    ['Luz'],
    xdkDataLum,
    optionsConfig('XDK: Luminescência', 'Lux')
  )
  drawTimeline(
    document.getElementById('timeline_turn_chartXdk4'),
    ['Temp.', 'Press.', 'Umid.'],
    xdkDataTPU,
    optionsConfig('XDK: Temp, Press, Umid', '')
  )
  drawTimeline(
...
function optionsConfig (title, subtitle) {
  return {
    chart: {
      title: title,
      subtitle: subtitle
